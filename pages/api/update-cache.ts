...
// Manual trigger for updating cache (for testing)
export default async function handler(
  ...
  try {
    console.log("Manually updating graph data...");
    ...
    console.log(
      `Found ${vouchers.length} vouchers and ${transactions.length} transactions`
    );
    // Generate graph data
    ...
    console.log("Graph data updated successfully in KV (manual trigger)");

    return res.status(200).json({
      success: true,
      message: "Graph data updated successfully (manual trigger)",
